...
Drupal.behaviors.greybox = function() {
  ...
  $('a.' + Drupal.settings.greybox.class_text).bind('click', function(){
    ...
    GB_WIDTH = dims[0] || Drupal.settings.greybox.gbwidth || Math.round(windowWidth - (windowWidth/100)*1);
    GB_HEIGHT = dims[1] || Drupal.settings.greybox.gbheight || Math.round(windowHeight - (windowHeight/100)*2);
...
function GB_hide() {
  $("#GB_window").animate({ height: 0, width: 0, opacity: 0 }, 350, 'linear', function() {
    $(this).hide();
  });
